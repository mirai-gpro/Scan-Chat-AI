...
console.log('\n④ 進行の判断を LLM に渡していない\n');
{
  ...
  /*
   * **訂正のあと、同じ質問をもう一度読ませる** (実機報告 2026-09-23:
   * 「訂正する → 80 → 体重の変化について が読上げされない」)。
   *
   * クライアント側は決定論的に正しい (`submitAnswer` → `applyQuestionToUI(next)` →
   * `sendModelTurn(next.id)` を遅延も抑制もなく実行・`sendModelTurn` に重複抑止は無い)。
   * 訂正すると**同じ依頼文が 2 度届く**ので、proactive audio が恒久 ON の 3.8 では
   * モデルが「もう読んだ」と判断して黙り得る。**判断させない**規定を置く。
   */
  ok('訂正で同じ質問を読み直す規定がある',
    /A-3\./.test(prompt) && /何度目であっても必ず読み上げる/.test(prompt),
    '訂正のあと次の質問が読み上げられない (モデルが「もう読んだ」と判断する)');
  ok('読んだかどうかをモデルに判断させない',
    /すでに読んだかどうかを自分で判断しない/.test(prompt),
    'A / A-2 と同じ流儀 — 進行の判断はモデルに持たせない');
}
