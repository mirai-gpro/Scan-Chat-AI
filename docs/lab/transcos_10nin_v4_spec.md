...
作成: 2026-09-15 / 更新: 2026-09-15 (Q1 / Q2 / Q4 裁定を反映)
状態: **設計。発注者承認まで実装しない**
...
> **発注者確認 Q3 (未回答・納品を止める)**: 10 名の `client_id` をどう用意するか。
> ①既存の診断ユーザーがいる ②今回用に採番する ③Wellfort 側で払い出す。
> S3 のキーが `user/{client_id}/…` なので、**これが決まるまで誰にも納品できない**。
> 解析と確認 (ドライラン) はこの回答を待たずに進められる。
...
| # | 内容 | 影響 | 章 |
|---|---|---|---|
| **Q3** | 10 名の `client_id` をどう用意するか | **納品の前提条件**。S3 のキーが `user/{client_id}/…` なので、**これが決まるまで誰にも納品できない**。①既存の診断ユーザーがいる ②今回用に採番する ③Wellfort 側で払い出す | §8 |
