...
**実物の ZIP が未受領 (= step 2 に入れない)。**

| # | 内容 | 前提 |
|---|---|---|
| 0 | **ZIP の中身を数える** — 人物フォルダの構成 / ファイル名の規則 / どのファイルがどの検査か / 問診 XLSX の**列見出しと選択肢**。**画面の「ZIP を選んで数えるだけ」で出す**ので、150MB の ZIP も PII もこの場に渡さずに済む (§14.1) | ZIP (管理者の手元) |
