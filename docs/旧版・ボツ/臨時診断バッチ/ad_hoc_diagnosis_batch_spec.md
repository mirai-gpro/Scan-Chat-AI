> **【旧版・参照禁止】この文書は 2026-09-14 に v1.1 へ全面差替えされた旧仕様（v1.0）である。**
>
> - **現行の正本は `docs/lab/ad_hoc_diagnosis_batch_spec.md`（v1.1）。**
> - ここの記述を実装・仕様判断の根拠にしない。**履歴調査にのみ使う。**
> - 現行と食い違う箇所を見つけても**ここを直さない**（現行側を直す）。
>
> 差替えの理由（v1.1 の決裁）:
> ①健診は ad-hoc 独自 XLSX parser ではなく **production HealthCheckup scan へ結線**する、
> ②Genoplan は **p10〜35 のみ**（全 208/210 ページ走査は禁止・cache は理由にならない）、
> ③問診 XLSX adapter は**実物 62 列の schema を正本に作り直す**、問診 PDF は**手動確認入力**。
