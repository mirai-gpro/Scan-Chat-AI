...
export interface MeasurementRow {
  ...
  value: string;
  /**
   * 検査日 (受領ファイルの各エントリが持つ `date`)。**受領データそのもの**で捏造ではない。
   * 【なぜ足したか・2026-09-25】各項目は複数回分 (過去〜今回) を持つ。以前は配列の
   * **先頭=最古**を日付なしで出していたため、2 年前の値が「今回の値」に見えていた
   * (Wellfort レビュー①③)。**今回 (最新日) の値**を採り、その日付を添えて
   * 「いつの値か」を紙面で辿れるようにする。取れないときは空 (プレースホルダを置かない)。
   */
  date?: string;
}
