...
export const CONFIG_SPECS: ConfigSpec[] = [
  ...
  { key: 'ui.report_new_days', type: 'string', group: '画面表示', label: '報告書「New」バッジの新着日数', default: '14',
    description: 'AI疾病予防報告書の受領日から何日間「New」を出すか (整数)。'
      + ' 決定論・受領日 (diagnosis_results.received_at) だけで判定し、閲覧済みかは見ない (端末をまたぐ記録を持たないため)。'
      + ' 0 以下・数値でない値は既定 14 に落とす。' },
  ...
  /*
   * 【削除: `ui.cancer_screening_not_included`・発注者指示 2026-09-18】
   * **admin が入力した当社の文を報告書の紙面に出す**ためのキーだった。
   * 受領 JSON に無い文言を報告書に出すのは捏造なので、キーごと廃止する。
   * (DB に行が残っていても参照するコードが無いので無視される。)
   */
