...
**【2026-09-15 実装済み】P2〜P5 を入れた。以下の「前景処理のまま」はもう当てはまらない。**

- **送信を押したら終わり。** 全ページに S3 のキーが揃っていれば
  `POST /api/scan/jobs` にジョブを積み、「送信しました」で画面を閉じてよい。
  **進捗も完了通知も作らない** (発注者判断「送信完了＝完了」)。
- 読み取りは **`GET /api/cron/scan-worker` (Bearer・毎分 cron・maxDuration 800s)**。
  deadline まで 1 枚ずつ進め、**時間切れでも `done_count` / `result_markdown` から続きを拾う**。
  `locked_until` + `attempts` の条件付き更新で**二重起動よけ**。
  完了時は前景と**同じ関数** (`scan-markdown.ts` / `saveScanResult`) を通し、
  `artifact_id` を書いて S3 の画像を消す。
- **1 枚でもキーが無い回は従来の前景処理へ落ちる** (S3 未設定・CORS 未設定・回線断を
  判別できないため。待たせても**必ず読める**方へ倒す)。
- **失敗はユーザーに出さない**ので `scan_jobs` に残し、`GET /api/admin/scan-jobs` が
  失敗・滞留を数え、**GitHub Actions の日次ワークフローが非 0 で赤くなる**
  (通知基盤は作らない)。**「引けなかった」を 0 件と混同しない** (見張りが緑のままになるため)。
- 検証 `npm run verify:scan-async` 33 件 (CI の A 層)・**退行注入 8 種**。
- **残る宿題 = 案B の後追い修正画面と再納品** (仕様書 §8 が「P4 に入る前に決める」とした未決事項。
  **実装で答えを出していない**)。
- **発注者の操作が 2 つ必要**: ①`supabase/migrations/20260910000010_scan_jobs.sql` の適用
  ②Vercel に `CRON_SECRET` を設定 (未設定でも `ADMIN_API_KEY` で叩ける)。

**以下は転換前の記録。実装は上の姿に変わっている。**
...
- **【EC 購入を伴わない招待に実データで使わせる = スペシャルアカウント 2026-09-15 実装】
  ...
  **デモ枠を共通基盤へリファクタしない** (稼働中の機能に波及する)。
  ...
  - **【ダッシュボードは「単品購入」の形にする 2026-09-15・発注者指示】仕様書 §14。**
    ...
    - **【複数年アップロード 2026-09-15・発注者指示】**`docs/lab/スペシャルアカウント_複数年スキャン_仕様書.md` §3。
      **1 回の送信 = 1 回分 (1 年分)。1 回の検査は 4〜10 枚の紙になるので、何枚まとめて送っても
      `test_artifacts` が 1 件になるのは正しい** (発注者確認)。足りなかったのは**回の区切り**だけ。
      送信前に「この 1 回分（1 年分）はこれで完了ですか？」、送信後に「別の年度を続けて登録する /
      AI 問診へ進む」を出す (`scan.astro` の `panel-done`)。**上限 5 年分**。
      **回数はサーバの事実 (`test_artifacts` の health_checkup 件数) から数える**
      (端末のカウンタだと別の端末で 1 に戻る)。次の回は**ページを開き直す**
...
| **`docs/lab/ad_hoc_diagnosis_batch_spec.md`** | **臨時診断バッチ (複数人分の検査データ ZIP を管理者が投入 → 分類 → 既存本番処理へ振り分け → 確認 → Elith 納品)**。**【v1.1 へ全面差替え 2026-09-14・発注者決裁】旧 v1.0 (150KB) は `docs/旧版・ボツ/臨時診断バッチ/` へ退避・参照禁止**。**位置づけが変わった** = 「10名専用の新しい診断方式」ではなく、**AI疾病予防報告書 単品購入 (タイプ2) の標準フローを管理者が一括投入できるようにし、標準に無い Genoplan を追加素材として渡す拡張ケース**。**遺伝子があってもタイプ1にしない**。臨時バッチは**オーケストレーション層**に徹する = 独自 OCR・独自 parser・独自 JSON schema・独自 ready 判定を持たない。**UI=wellfort-site / 処理=Scan-Chat-AI** (§4)。**v1.1 で覆った 3 点 (旧 v1.0 の記述を根拠にしない)**: ①**健診** — `health-checkup-xlsx.ts` を `HealthCheckupData` の正経路に**しない**。全10名に健診PDFがあるので **健診PDF → 既存 `scanImageToParsed()` → 本番 HC finalize → HealthCheckupData** へ結線する (§6.2/§6.5)。XLSX は照合・Golden の補助資料へ降格。**`elith-hc-merge` を ad-hoc から通常モードで直接呼ばない** (通常運用では part で元画像・finalize で JSON を S3 へ書くため、**Human Review 前に実書き込みが起きる**)。finalize を**副作用のない共通 core** へ抽出し通常経路と ad-hoc が同じ core を使う。**ad-hoc 独自の HC JSON builder を作らない**。②**Genoplan** — **p10〜35 inclusive の 26 ページだけ**。全 208/210 ページ走査は禁止で、**キャッシュがあることは不要ページを送ってよい理由にならない** (§8.4)。required page set の**正本は `src/lib/elith-genetic.ts`**、ad-hoc の受付拒否・complete 判定・retry・status/plan 応答が同じ定数を使い、**wellfort-site に `10`/`35` を独立の正本として持たない** (`elith-batch.astro` の from/to 既定値は運用UIの初期値であって ad-hoc の正本ではない)。**complete は「登録済み行が全部 done」では不足** — required set の各ページについて**行が在り status=done** のときだけ完了 (通信断で行すら無いページを完了に数えない)。p<10 / p>35 は `scanGeneticPage()` 呼出前に**テスト可能な 4xx で reject** (silent skip にしない)。③**問診** — 実物の 62 列 XLSX の列見出しは「身長を教えてください。（172cmの場合…）」のような**完全な設問文**で、現行 `questionnaire-map.ts` の短縮キー (`身長` 等) は正規化後の完全一致では**当たらない**。→ **実物の列見出しを正本に外部列 → production `question_id` の明示変換契約を作り直す** (§7.4)。fuzzy/LLM 推測は禁止・値変換は exact か仕様化した決定論変換だけ・**複数回答を「最初の1件」に落とさない**・表現できない値は切り捨てず **`needs_review`**・対象外列 (がんリスク専用/AI疾病予測専用/同意等) は **`ignored_by_spec`** と明示。**問診PDF (2名) は `normalizePdfText()` を本経路で使用禁止** — 回答が radio/checkbox の**視覚的な選択状態**で、text 抽出では未選択肢も列挙されるため次行取得方式では正答を保証できない。**管理者が PDF を見て production の question_id / AnswerValue へ手動確認入力し二重確認**する。**LLM に選択状態を推測させない**。**KEEP (調査で適合を確認済み)**: 遺伝子の `test_date` は `gFile.test_date ?? null` で **today fallback を使っていない** (汎用 `elith-genetic-merge.ts` の today fallback は今回の修正対象外)／ad-hoc は **`BloodTestData` を生成しない** (今回 ZIP に Demecal CSV が 0 件。健診PDF 内の血液値は `HealthCheckupData` の一部として読む。将来も **LLM ではなく `parseBloodCsvRowsStrict()`**)。**実データ (2026-09-10 受領 ZIP)**: 10名 / 38ファイル / PDF 22 / XLSX 15 / DOCX 1 / Genoplan PDF 10件 (208 or 210p) / 共通問診XLSX 8名 / 問診PDF 2名 / 健診PDF 10名 / 健診系XLSX 5名 / **Demecal CSV 0件**。**実在役員の氏名・問診回答・健康情報を repo へ fixture として commit しない** — repo に残すのは**匿名化・合成データ**と、実物から得た**列 schema・値 domain・mapping 規則**だけ (§7.4)。**DB は触らない** = 新 migration 原則不要・DROP / rollback / history repair をしない。**成功条件は API 200 でも JSON 生成でもテスト PASS でもない** — **10名全員について実データが正しく反映された AI疾病予防報告書が完成し、トップセールスで使える状態になること** (§22)。**10名一括の前に必ず 1名で E2E** (健診PDF が本番 HC を通った / Human Review 前に S3 へ書かれていない / Genoplan の LLM call が p10〜35 だけ / 3 format が同一 client_id / dry-run / Elith 実納品 / 報告書へ遺伝子が反映)。**【実装状況 2026-09-14】v1.1 の §7 のうち 4 本を実装済み**: ①仕様書差替え ②Genoplan page policy (正本=`elith-genetic.ts` の `GENOPLAN_V1_REQUIRED_PAGES` / 受付は `400 page_out_of_range` / complete は必要集合から引く / status API が集合を配り UI は数値を持たない) ③健診PDF 結線 (`elith-hc-merge` の finalize を**副作用のない共通 core** `src/lib/elith-hc-finalize.ts` へ抽出し 通常経路と ad-hoc が同じ core を呼ぶ。ad-hoc は `scanImageToParsed` を直接呼び **解析段階で S3 へ書かない**。`dateSource==='today'` は捨てて未解決にする。生 Markdown は DB にも納品 JSON にも残さない) ⑤問診PDF の手動確認入力 (`questionnaire-manual.ts`・自動 text parser は本経路から外した・**二重確認が済むまで納品に使わない**・入力を差し替えたら確認が外れる)。**④問診 XLSX adapter も実装済み (2026-09-14・発注者が実物 8 件の 62 列を確認し写像契約を提示)**: 正本=`src/lib/ad-hoc-diagnosis/external-form-contract.ts` に**実物の完全な設問文の見出し 62 本**と 列ごとの disposition・変換規則を持つ (設問へ写像 43 列/35 設問・`ignored_by_spec` 10 列 (53〜62)・PII 3 列 (4,5,7)・Forms 管理列 3 列・完了時刻/性別/生年月日 3 列)。**列の位置で引き、schema が 1 文字でも違えば 1 列も読まない** (ずれたまま読むと値が別の設問へ入り、しかも全部「未知値」に見えて原因が分からない)。正規化は互換文字だけ (NFKC + 波ダッシュ `～`/`~`→`〜` + 空白)。**fuzzy/LLM 推測なし・未知値を「その他」へ寄せない・複数回答は 1 件でも未知なら設問ごと `needs_review`・`E-TYPE` は単一選択なので外部の複数回答から先頭を採らない・`SL-STRESS` は 1〜10 の整数のみで clamp しない・`E-SITTING` の「N時間以上」から区間を作らない・`F-RICE` は明示 alias のみ**。`EXAM-TYPE` は 62 列に申告列が無いので**申込・バッチ文脈から seed** (`AI疾病予防のみ` + `遺伝子検査（唾液検査）のみ`。**Genoplan が増えてもタイプ2 のまま**)。**today fallback を 2 か所で潰した**: ①`normalizeExternalFormRow` の年齢基準日が `new Date()` だった → 完了時刻が解決したときだけ算出し、解決しなければ `age=null` + `needs_review` ②**`buildElithInterviewJson()` は `completedAt` が無いと実行時刻を完了日とみなし `test_date` を今日にする** (`interview-export.ts:250`)。通常のアプリでは正しいが本経路では捏造なので、**production 側の既定は変えず** `questionnaireIsUsable()` で「日付が確定していない/schema が違うものは納品しない」と閉じた。**mapped / ignored_by_spec / needs_review を別々に数える** (混ぜると正常な除外と確認待ちが区別できない)。**件数だけ保存**する (`unmapped[].detail` には回答値が入るので永続化しない)。検証 `verify:ad-hoc-external-form` 147 (**fixture は合成データのみ**)・退行注入 9 種。**DB は 3 コミットとも migration 0 件** (`ad_hoc_diagnosis_pages` を健診ページと問診手入力に使い回し、event 名も既存の `parsed`/`confirmed` に寄せた = CHECK 制約に触れない)。検証は `verify:ad-hoc-genoplan-pages` 45 / `verify:ad-hoc-health-wiring` 51 / `verify:ad-hoc-questionnaire-manual` 54 / `verify:ad-hoc-external-form` 147 を新設し CI の A 層へ追加 (**退行注入 23 種**で落ちることを確認済み)。**⑥ production 分岐の適用と納品ゲート (2026-09-14・最終指示書 §3〜§5)**: Phase A の 62 列写像だけでは **production の問診票として成立していなかった** — `QUESTIONS[].when` の分岐が最終 answers へ当たっておらず、`buildElithInterviewJson()` は渡された answers をそのまま出す。→ `src/lib/ad-hoc-diagnosis/questionnaire-branch.ts` で **production の `resolvePath()` を共用**して分岐を適用 (**独自 when をコピーしない**。同じ条件式を 2 か所に置くと問診票を直したとき片方が腐る)。**inactive な設問は納品 answers に入れない** (件数だけ `ignored_by_branch`)。ただし**黙って医療情報を捨てない** = 「なし/摂取していない/空欄/0」は branch ignore、**`E-TIME`/`E-TYPE` は値の中身に関わらず branch ignore** (§3.2 の明示例)、**それ以外の実質的な値 (実薬名・喫煙本数・飲酒量・治療状況) は矛盾として `needs_review`**。**納品ゲートを反転した (最重要)**: `questionnaireIsUsable()` は **schema OK ∧ completedAt resolved ∧ mapped>0 ∧ 未解決の active な要確認 = 0** の全部を満たすときだけ true。**一部だけ写像できた `LifestyleQuestionnaireData` を完成品として出さない** (以前の「1 項目でも写像できていれば納品」は撤回)。解決は **PDF 用に作った `questionnaire-manual.ts` / `questionnaire-answers` API / confirmation を再利用** (新しい自由形式 parser を作らない)。`deterministic answers + confirmed manual overrides → 分岐 → final answers`。**保存するのは `review_question_ids` (question_id だけ) と件数**で、未知の生回答値は永続化しない。plan と deliver は **`resolveQuestionnaireForDelivery()` の 1 か所**を通る (片方だけ分岐を当てると画面と納品物が食い違う)。**実測で見つけた既存バグも直した**: `assertNoPiiKeys()` の deny-list は `name` を部分一致で見るため **設問 id `M-NAME` に当たり「服薬あり」の人物の保存が丸ごと throw していた** → 除外は**実在する設問 id との完全一致のみ** (`display_name` 等は今までどおり止める)。UI は wellfort-site の「5b. 問診の手入力・要確認の解決」へ統合 (XLSX は**要確認の設問だけ**出す・matrix は行ごと・原本を開く)。検証 `verify:ad-hoc-questionnaire-branch` 104 を新設 (**退行注入 10 種**)。**DB migration 0 件**。
**⑦ E2E 確認用「1 JSON だけ S3 へ書く」経路 (2026-09-14・発注者指示)**: 1 名の E2E 確認として、生成済みの `HealthCheckupData` 1 ファイルだけを実際の Elith 納品先へ置いて中身を原本と突き合わせる口。正本=spec §11.4。**正式なバッチ納品ではない** — `batch.status` を `completed` にしない・他 format を `exported` にしない・`exported` イベントを出さない (通常の一括納品から見るとバッチは引き続き未納品)。**通常納品の規則は 1 文字も変えていない** — 「1 人でも未完成なら部分納品しない」を緩めるのでなく**経路を分けた** (`exportSingleDeliveryFile()` / `POST /export-one`)。E2E では部分納品禁止に引っかかるのが正常なので、例外分岐を足さない。**新しい JSON を作らない** = 納品ファイルの組み立ては `buildSubjectDelivery()` **1 か所だけ**で通常の dry-run / 本番納品と同じこれを呼ぶ → PUT する body は通常納品時の JSON と**バイト単位で同じ**。**安全側の検査は 1 つも外していない**: env 2 本 (`AD_HOC_ELITH_WRITE_ENABLED` / `AD_HOC_ELITH_WRITE_TARGET`) の一致は `checkWriteTarget()` を通常経路と共有・`validateDeliveryKey()`・`HeadObject` で既存があれば中止 (上書きも削除再実行もしない)・`IfNoneMatch:'*'` の create-only・PUT 後に読み戻し。加えて **Executive 紐付け必須 / ちょうど 1 件でなければ書かない / 操作者が原本で確認した `expectTestDate` (省略不可) と `test_date` が一致しなければ書かない (today 混入の番人) / JSON 本文の `format_id`・`client_id`・`test_date` も検査**。**「S3 に 1 件在った」では PASS にしない** — API は実際に PUT した本文を返し、管理画面 (6b) から保存して原本の健診 PDF と 身長/体重/BMI/血圧/HbA1c/AST/ALT/γ-GTP/HDL/TG/LDL を突き合わせる (**本文はログに出さない・repo に入れない**)。監査は既存 event 名 `override` + `detail.kind='e2e_single_write'`。検証 `verify:ad-hoc-single-write` (**退行注入 10 種**)。**DB migration 0 件**。
**⑧ PII ゲートの二重実装を潰した (2026-09-14・実データ E2E で顕在化)**: `classify-entry` が `normalized_payload に禁止キー: upsertFileByEntryIndex[0] [M-NAME]` で落ちた。**真因は PII 判定を 2 か所で実装していたこと** — 組み立て側 (`normalized-payload.ts`) は production の設問 id を完全一致で除外していたのに、**書き込みの扉 `store.ts` の `guardPayload()` / `updateFile` が `assertNoPiiKeys()` を除外なしでもう一度呼んでいた**ため、`M-NAME` が `name` の部分一致で再び拒否された。→ **`assertNormalizedPayloadSafe(payload, where)` を正本として 1 本公開**し、組み立て側 (`buildQuestionnairePayload` / `buildEntryPayload` / `buildHealthCheckupPayload`) も書き込みの扉も**全部これを通す**。**`QUESTION_ID_KEYS` を store 側へ複製しない** (複製すると片方だけ更新されて同じ事故が再発する)。**安全性は緩めていない** — 除外は**実在する設問 id との完全一致だけ**なので `display_name` / `full_name` / `file_name` / `name` / email / birth / company と、**production に無い `UNKNOWN-NAME` のような id** は今までどおり止まる。検証 `verify:ad-hoc-payload-gate` 45 を新設 (**Supabase をスタブに差し替えて `upsertFileByEntryIndex` / `replaceFiles` / `updateFile` を実際に呼び、DB 直前まで到達することを見る**。退行注入 4 種: 扉が除外なしで呼ぶ / 除外を部分一致に緩める / store へ id 一覧を複製する / 組み立て側の除外を外す)。**DB migration 0 件・通常納品 / S3 write / 問診 mapping の仕様変更なし**。
**v1.1 §7 の 5 本 + 最終指示書 §3〜§5 + E2E 1 件書き出し すべて実装済み。残るは本番環境での実行 (資格情報・ZIP が要る)**。 |
...
| **`docs/scan/スキャン非同期処理_仕様書.md`** | **【送信後はバックグラウンドで読み取る。P0〜P5 実装済み 2026-09-15。残るは発注者による migration 適用と `CRON_SECRET` 設定】発注者判断 2026-09-10「S3 に置いて定期的に削除で進めて」。** 画像を全ページ S3 へ / 削除は既存ライフサイクル(1日)＋読了時削除の二段 / `diagnosis.scan_jobs` / ワーカーは 1 起動で回せるだけ回す / **Vercel の上限は 60s でなく 300〜800s・Cron 最小間隔はプラン依存**(一次資料つき) / 段階 P0〜P5 / **発注者確認 4 件** |
...
| job | 中身 | 実測 | 状態 |
|---|---|---|---|
| `static-required` | astro check / build / **A 層 14 本** (2026-09-10 に臨時診断バッチの 3 本を追加) | 60s + npm ci | **required** |
