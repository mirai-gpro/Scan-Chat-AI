...
- **【2026-09-18 確定・発注者指示】Gemini 3.8 へ上げるのは決定事項。先に上げる (下の段取りを上書き)。**
  - 発注者:「**3.8 へ上げるのは決定事項だよ？**」「逆に **3.1 でしか確認できない項目はあるの？**」
  - **答え = 無い。** ①発話の挙動 (自発発話・完了宣言・ト書きの読み上げ) は **3.8 で proactive audio が
    恒久 ON になり性質が根本から変わる**ので、3.1 での実証は捨てるモデルの挙動を測るだけ
    ②音声回答の選択肢推論は **`scan.model` (REST・別モデル)** なので 3.8 と無関係
    ③決定論の検査 (`verify:*`) はモデル非依存。
  - **切替はコード変更ゼロ**: `live-token.ts` も `live-controller.ts` も `getLiveModel()` 経由。
    **コード既定を `gemini-3.8-live` に変更済み**。ただし **DB 値が勝つ**ので、
    `diagnosis.app_config` に `live.model` の行があれば **admin で書き換えるか行を消す**こと。
  - **最大の Go/No-Go = 指定した質問を 100% 読み上げるか。** proactive audio は
    「関連しないと判断したら応答しない」機能なので、**質問の読み上げ自体をサボると問診が成立しない**。
  - **`proactivity` は渡さない** (`proactive_audio: false` はエラー)。現行コードは渡していない。
    `verify:interview-live-transport` が「渡していないこと」を見張る。
  - **未確認 (やれば 1 回で分かる)**: SDK **2.6.0** で 3.8 が通るか (最新 2.23.0) /
    **v1alpha** の ephemeral token で 3.8 が通るか (公式は v1beta)。
    **どちらも駄目なら接続エラーで即分かる**ので切り分けは失われない。駄目だったときだけ P1/P2 を行う。
  - 戻すのも即時 (`live.model` を `gemini-3.1-flash-live-preview` へ)。
- **【2026-09-17・上書きされた段取り】④⑤ の修正に Gemini 3.8 は不要。移行は最後に独立した PR で行う。**
...
- **【2026-09-17 確定・発注者指示】利用者が自分でマイクを切れるようにする。これは禁止に当たらない。**
  正本 `docs/interview/AI問診_仕様と設計原則.md` §3.1。
  - 発端 = 実機で**テレビの音をかなり拾う**。マイクは開始から中止まで送りっぱなしなので、
    周囲の音が「回答」として確定し得る。**静かな場所を想定し、それ以外は切ってもらう。**
  - 禁じているのは**プログラムが勝手に止めること** (半二重ゲート・VAD 誤検出よけ・独自の状態機械)。
    **利用者のタップは UI であってターン制御ではない。**
  - **境界は機械で固定** (`scripts/verify-interview-live-transport.mjs`): 切り替えの呼び出し元は
    **マイクボタンの click 1 か所だけ**。`handleServerMessage`・マイク送信のコールバック・
    AI の再生終了からは呼べない。`mic-gate.ts` はタイマーもイベント購読も持たない。
    **退行 5 種を注入して名指しで落ちることを確認済み。**
  - **止めるのは送信だけ** (発注者裁定 A・18%・「送るのをやめるだけ」)。端末のマイクは掴んだまま
    = 戻すときに許諾ダイアログが出ない。代わりに **iOS の録音インジケータは点いたまま**。
    **AI の読み上げは止めない。** 切っている間は「そのまま話して回答できます」を引っ込める。
    **前回の選択は持ち越さない** (問診を始めるたびオン)。
  - 見た目 = 質問の下の帯 (文言「周囲の音を拾う場合、マイクをオフに」＋右端に 44×44px)。
    **3 秒周期で点滅** (0.33Hz・WCAG 2.3.1 の 3Hz を大きく下回る・`prefers-reduced-motion` で停止)。
    **中止・スピーカーは 1px も触っていない。** 青は `.mic-gate` だけのトークンで
    `tailwind.config.mjs` の brand は変えない (`.report-sheet` と同じ流儀)。
  - **未解決 = 拾った音が回答として確定し得ること自体** (`interpretVoiceAnswer` は自由記述を逐語・
    スライダーを任意の数字で採る)。マイク OFF は利用者の自衛手段であって、こちらは**別途の判断待ち**。
...
| `docs/subscription/kit_lifecycle_and_handoff_management_spec.md` | **検査キット 出荷・進捗・データ受渡 統合管理仕様(サブスク駆動)**。プラン×キット×発送タイミング/タカセ定期出荷/ライフサイクル状態機械+AI問診促し/進捗駆動の各社受渡・Elith作成指示。**§4.1.1=LAiF上りCSV(AI疾病発症予測 入力フォーム 約158項目)の写像仕様＋生成フロー**(健診スキャン+AI問診+基本情報を集約=スキャンフローに足さない別export・整理番号/生年月日は要確認)。**§4.3.1=権利と条件(Elith AI診断の自動生成トリガ・確定2026-09-24)**: 権利=契約 plan_composition から自動判定(4プラン+単品・名称では判定しない)・条件=その回の予定検査が全部揃ってから発火(回窓連動の問診・HealthAgeは非ブロッカー)・**発火=毎日23:00 JST の cron `GET /api/cron/elith-deliver`(vercel.json `0 14 * * *`=14:00 UTC・Elith が深夜バッチ取込のため)。`skipDelivered:true` で納品済みの回は再送しない(冪等)。admin ボタンは従来どおり毎回ラップ**・**前提=契約テーブル(subscriptions/plan_compositions*)が実在0件で埋め込みが自動判定の先行条件**・埋め込み前は cron が拾えるのは単品/スペシャルのみでコースプランは admin 手動。**§4.3.1 E=契約テーブル埋め込みの進捗(2026-09-24)**: DDL済(wellfort `20260910000010`)・マスタseed済ファイル(`seed-kit-composition.sql`)・**遡及backfill用意(`backfill-subscription-composition.sql`=既存契約を version=1 へpin・NULL行のみ冪等)**・新規契約pinはcreate-order改修(§8.3)へ・**DB適用は発注者(staging限定/Prod NO-TOUCH)** |
...
| job | 中身 | 実測 | 状態 |
|---|---|---|---|
| `static-required` | astro check / build / A 層 | 60s + npm ci | **required** |
