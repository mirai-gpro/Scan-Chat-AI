...
| **`docs/lab/ad_hoc_diagnosis_batch_spec.md`** | **臨時診断バッチ (複数人分の検査データ ZIP を管理者が投入 → 分類 → 構造化 → 確認 → Elith 納品) のクロスシステム仕様**。**UI=wellfort-site / 処理=Scan-Chat-AI**(2026-09-10 発注者確定・§4)／ZIP は presigned PUT で Vercel を通さない(§5.2)／**ZIP・XLSX を読む手段は決定** (2026-09-10 Phase D-0・一次資料で 11 本を実測比較)＝**ZIP=`@zip.js/zip.js`**(依存0/既知脆弱性0/BSD-3。**ブラウザ `BlobReader` とサーバ Range GET を同じ ZipReader で賄えるので ZIP 解釈が 1 本に統一でき検査も 1 か所**・CP932 可) / **XLSX=`read-excel-file`**(MIT/脆弱性0/両対応。**セル値を `Date` で返す**=日付シリアルの判定をライブラリが持つ。`test_date` は納品パスと🎯照合を決めるので自前判定で静かに 1 日ずらせない)。**自作は不採用**・**医療データなので独自 ZIP parser を第一選択にしない**(§5.3)。**SheetJS `xlsx`/`node-xlsx` は採用不可**=advisory 本文が**「npm に修正版が存在しない」と明記**し実測でも npm は 0.18.5(2022-03-24) 停止、HIGH 2 件のうち Prototype Pollution は**「細工ファイルを読むとき」=本機能そのもの**(§5.3.6)。`adm-zip` も除外(**最新版がまだ影響下で修正版が無い** symlink 上書き)。`jszip` は全体メモリ展開で不可。**【Phase D 着手済み 2026-09-10】ZIP 基盤・S3 Range Reader・分類・健診 XLSX を実装**(`src/lib/ad-hoc-diagnosis/{keys,archive,classify,fingerprint,health-checkup-xlsx}.ts`・検証 `npm run verify:ad-hoc` = archive 85 + parse 113)。**問診 parser は O5 未解消のため未着手**。**ZIP を触るのは `archive.ts` の中だけ**(ブラウザ側は同じ ZipReader に BlobReader を渡す=検査が 1 か所)。**実測で仕様を 2 か所訂正**: ①**`cp932` は WHATWG のラベルではなく `TextDecoder` が投げる**。zip.js は `getEntries()` の中で decode するので**投げると ZIP を開けなくなる**(文字化けで済まない)。→ `pickFilenameEncoding()` が `shift_jis`/`windows-31j`/`ms932` を probe し、駄目なら既定に委ねる=**実行環境の測定に依存しない**(§5.4) ②**Range の終端を +1 しても zip.js は壊れない**(要求した長さぶんしか使わない)=**ライブラリは守ってくれない・範囲が静かにずれるだけ**。→ `rangeHeaderFor()` で算術を直接固定し `assertExactLength()` が**短い側だけでなく長い側も投げる**(§5.3.7.1)。**`read-excel-file` の 5 点は実測済(§5.3.8.2)**: 組み込み/ユーザー定義とも日付書式は `Date` で返る・**1904 date system を honour する**(自前変換不要)・空欄は `null` で 0 と区別できる・和文見出しは化けない。**書式の無い数値は `number` のまま**=これが「勝手に日付化しない」対象で、**同じシリアル値 46110 は 1900 で 2026-03-29 / 1904 で 2030-03-31 = 決め打ち変換は約4年ずれる**を実測で固定。**v9 の既定 export は「シートの配列」**(行の配列ではない)・**シート番号を決め打ちしない**(見出し一致数で選ぶ)／ブラウザの逐次 SHA-256 は wellfort-site `src/scripts/ad-hoc-diagnosis/zip-digest.ts`・検証 `npm run verify:zip-digest` 40/40 (**`arrayBuffers` を `--expose-gc` 付きで測る** — 参照を数える方式も `heapUsed` も**貯める実装を検出できなかった**。実測の分離 0.3MB / 40.3MB)／**【v0.5 で 3 点追加・発注者指示】ブラウザの SHA-256 は `@noble/hashes` の逐次 API** (`sha256.create()`→`update()`→`digest()`)。**ZIP 全体を `arrayBuffer()` 化して `crypto.subtle.digest()` へ渡すのは禁止**(`SubtleCrypto.digest()` は BufferSource 1 個の一発 API で `update()` を持たない=streaming にならない)。**import は `@noble/hashes/sha2.js` で `.js` 必須**(exports に `"./sha2"` が無い)。**ticket 発行前の `source_sha256` と再読込後の同一性照合を同じ実装で**(別実装だと再開が黙って新規バッチになる)・**DB 設計は変更不要**(§11.6)／**サーバ側 ZIP は custom `Reader`→AWS SDK `GetObjectCommand` の `Range`。presigned GET は使わない**(**Range は両端を含む閉区間**=終端に `offset+length` を書くと 1 バイト多く読む・§5.3.7.1)／**`read-excel-file` は Scan-Chat-AI サーバ側だけ**。実物 XLSX で Excel日付/カスタム日付書式/1900・1904/空欄と0/日本語ヘッダー を実測し、**判定できないカスタム書式を勝手に日付化しない**(数値のまま `needs_review`。変換は 1900/1904 の決め打ちが要り**4 年ずれた日付を静かに作る**=捏造)・**`test_date` が確定しない人物は納品しない**(§5.3.8.1)／**`.xls` は受入対象外**(OLE2 で ZIP/XML でない)／**ZIP 全体をメモリ展開しない**(サーバ=S3 Range GET / ブラウザ=`File.slice()`・§11.5)／人物識別と分類を分離・**client_id は新規 UUID**(§6/§13)／**再開時の人物↔client_id は内容ハッシュだけから作る非可逆 `subject_fp` で結び直す**(氏名・フォルダ名を材料にしない。**fp は一意識別子でなく検索キー**=通常 INDEX・ヒット件数で 0=unmatched/1=match/2以上=fp_collision 全件 needs_review・§6.2)／**presigned PUT のサイズ防御は署名に依存せず多層**(ticket上限＋アップロード後 HeadObject＋展開上限。既存コードの「Content-Length も署名固定」というコメントは `signableHeaders` の実装と食い違う=未確認・§5.2.1)／健診 PDF+XLSX の二重納品を防ぐ(§12)／**既存 5 種 `GATING_FORMAT_IDS` を変更せず案件別 `required_formats` を持つ**(§15.2)／ウェルネス年齢は既存関数を呼ぶだけ・**全員算出可能と仮定しない**(§16)／**操作者識別の正は `actor_user_id`(uuid)**=wellfort-site が `/auth/v1/user` で検証した user.id を**サーバ側で注入**・**ブラウザ body の値は信用しない**(§21・O10 CLOSE)／**ZIP サイズは申告値と実測値を別列**(`declared_source_size` NOT NULL / `source_size` NULL 可)=batch 行は PUT 前に出来るので 1 列だと**申告値を実測値として保存**することになる(§23.1)／未確定 **7** 件は §28.2 に分離(O5=問診の実列。**問診 parser は O5 解消まで着手しない**)。**O1/O8/O10 は解決済**(§28.1) |
...
| job | 中身 | 実測 | 状態 |
|---|---|---|---|
| `static-required` | astro check / build / **A 層 13 本** (2026-09-10 に臨時診断バッチの 2 本を追加) | 60s + npm ci | **required** |
