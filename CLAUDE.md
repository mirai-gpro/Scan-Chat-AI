...
- **【EC 購入を伴わない招待に実データで使わせる = スペシャルアカウント 2026-09-15 実装】
  ...
  **デモ枠を共通基盤へリファクタしない** (稼働中の機能に波及する)。
  ...
  - **【ダッシュボードは「単品購入」の形にする 2026-09-15・発注者指示】仕様書 §14。**
    ...
    - 進捗 = **AI問診 → 検診・人間ドックのアップロード → 報告書** の 3 ステップ
      (`SinglePurchaseProgressCard`)。**検査結果より上**に置く (この入口の「次にすること」が本題)。
    - 検査結果は **人間ドック / 健康診断 の 1 種だけ** (他 4 種は構造的に来ない)。
      主要導線 (AIスキャン / AI問診) は**出さない** (進捗カードと行き先が同じ)。
      **コースプランの画面は 1 px も変えない** (5 枚のときの grid 文字列を据え置き)。
    ...
    - 検証 `npm run verify:single-purchase` 38 件 (CI の A 層)・**退行注入 12 種**。
