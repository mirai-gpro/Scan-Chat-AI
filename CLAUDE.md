...
| **`docs/lab/ad_hoc_diagnosis_batch_spec.md`** | **臨時診断バッチ (複数人分の検査データ ZIP を管理者が投入 → 分類 → 構造化 → 確認 → Elith 納品) のクロスシステム仕様**。**UI=wellfort-site / 処理=Scan-Chat-AI**(2026-09-10 発注者確定・§4)／ZIP は presigned PUT で Vercel を通さない(§5.2)／**ZIP・XLSX を読む手段は決定** (2026-09-10 Phase D-0・一次資料で 11 本を実測比較)＝**ZIP=`@zip.js/zip.js`**(依存0/既知脆弱性0/BSD-3。**ブラウザ `BlobReader` とサーバ Range GET を同じ ZipReader で賄えるので ZIP 解釈が 1 本に統一でき検査も 1 か所**・CP932 可) / **XLSX=`read-excel-file`**(MIT/脆弱性0/両対応。**セル値を `Date` で返す**=日付シリアルの判定をライブラリが持つ。`test_date` は納品パスと🎯照合を決めるので自前判定で静かに 1 日ずらせない)。**自作は不採用**・**医療データなので独自 ZIP parser を第一選択にしない**(§5.3)。**SheetJS `xlsx`/`node-xlsx` は採用不可**=advisory 本文が**「npm に修正版が存在しない」と明記**し実測でも npm は 0.18.5(2022-03-24) 停止、HIGH 2 件のうち Prototype Pollution は**「細工ファイルを読むとき」=本機能そのもの**(§5.3.6)。`adm-zip` も除外(**最新版がまだ影響下で修正版が無い** symlink 上書き)。`jszip` は全体メモリ展開で不可。**【実装完了 2026-09-10・v1.0】ZIP投入→S3→解析→人物分離→分類→健診/遺伝子/問診→人物整合→ウェルネス年齢→管理者確認→Elith JSON→納品セット→dry-run→管理UI まで通した (spec §31)**。**処理=`src/lib/ad-hoc-diagnosis/{keys,ticket,archive,classify,fingerprint,health-checkup-xlsx,questionnaire-map,questionnaire,pipeline,store,service}.ts` + API 9 本 `src/pages/api/admin/ad-hoc-diagnosis/*` / UI=wellfort-site `/admin/ad-hoc-diagnosis` + 中継 `[...path].ts`**。検証 `npm run verify:ad-hoc` (archive 85 + parse 113 + e2e 92) と wellfort-site `verify:zip-digest` 40。**ZIP を触るのは `archive.ts`・`read-excel-file` は `health-checkup-xlsx.ts`・DB 6 表は `store.ts` だけ**。**既存を再利用**: 遺伝子=`scanGeneticPage`(新promptを作らない)・問診=`buildElithInterviewJson`・年齢=`computeWellnessAge`・整形=`sanitizeMeasurementsForDelivery`。**納品は既定 dry-run**・**既存 `GATING_FORMAT_IDS` は不変**で案件別 required を持つ。**操作者は中継が `/auth/v1/user` で検証した user.id を注入**しブラウザ body の actor は削除して転送。**実測で仕様を訂正**: ①**`cp932` は WHATWG のラベルでなく `TextDecoder` が投げる**(zip.js は `getEntries()` 内で decode するので**ZIP を開けなくなる**) → probe して駄目なら既定に委ねる ②**Range の終端を +1 しても zip.js は壊れない**=**ライブラリは守ってくれない**・範囲が静かにずれるだけ → `rangeHeaderFor()` で算術を固定し `assertExactLength()` が**長い側も投げる** ③**`S3RangeReader` は末尾をまたぐ要求を丸める**(`min(length,size-offset)`・`offset>=size` は空)。**`read-excel-file` の 5 点は実測済**: 組み込み/ユーザー定義とも日付書式は `Date`・**1904 date system を honour**・空欄は `null` で 0 と区別可・和文見出し化けなし。**書式の無い数値は `number` のまま**=勝手に日付化しない対象で、**同じシリアル値 46110 は 1900 で 2026-03-29 / 1904 で 2030-03-31 = 決め打ち変換は約4年ずれる**を実測固定。**v9 の既定 export は「シートの配列」**・**シート番号を決め打ちしない**。**実装中に直したもの**: ワークブックの二重読みで片方の失敗が道連れ→1回読んで両方に使う / `sanitizeMeasurementsForDelivery` の戻り値は `{kept,anomalies}` (配列でない) / 単一選択の設問まで配列にしていた (`multi` のときだけ配列が正) / 設定未了を 500→**503 と理由**／ブラウザの逐次 SHA-256 は wellfort-site `src/scripts/ad-hoc-diagnosis/zip-digest.ts`(**`arrayBuffers` を `--expose-gc` で測る** — 参照を数える方式も `heapUsed` も貯める実装を検出できなかった。実測の分離 0.3MB / 40.3MB)／未確定 **6** 件は §28.2 に分離。**O1/O5/O8/O10 は解決済**(§28.1)。**O5(問診の実列) は発注者から 62 列の提示を受けて CLOSE** — 明示の写像表 `questionnaire-map.ts` を実装し**fuzzy な LLM 推測は使わない**・未対応は `unmapped` で**1項目未対応でも人物を落とさない**。**【分割分類 2026-09-12・Phase B2.1・spec §32】159MB/38ファイルの ZIP が `maxDuration=800` でも タイムアウト → **timeout を伸ばさず 1 リクエスト = ZIP 内 1 ファイル**に割った (`classify-plan`/`classify-entry`/`classify-finalize`・旧 `classify` は残す・**Executive 専用でなく汎用でも使える**)。**リクエストをまたいで指すのは Central Directory の序数(整数)だけ** — path/元ファイル名/人物フォルダ名は**保存も応答も例外メッセージもしない**。**本丸は後工程** = `process`/`health-age`/`export`/`GET /file` から `analyzeOpened()` を外し、分類時に保存した `normalized_payload` から復元する(残っていれば分割の意味が消え、**小さい ZIP では再現しない**)。payload は allow-list で組み**書き込み前に deny-list で検査して throw**(`Date` セル/PII 見出し列/`notes`/`unmapped` を落とす・遺伝子は null)。**行は `(batch_id, archive_entry_index)` で冪等**・**plan は人物行を作り直さない**(cascade で Executive の紐付けと遺伝子ページが消える)・**採用外のエントリも行を残す**(finalize が「未処理」と区別できず永久に完了しない)。**ZIP は上げ直さない**(`?batchId=` と「続きから再開」)。migration は前進のみ `20260912000010`(両列 nullable=後方互換・**先に Production Scan DB へ適用してから deploy**)。検証 `verify:ad-hoc-chunked` 176 / wellfort-site `verify:ad-hoc-chunked-ui` 40・退行注入 17 種。**Production 適用とバッチ再実行は未実施**・**Elith final write は OFF のまま** **【Range GET の粒度 2026-09-12・Phase B2.2・spec §32.11】分割しただけでは足りなかった** — 本番の最初の `classify-entry` が `FUNCTION_INVOCATION_TIMEOUT`。**原因は解析でなく S3 の I/O 粒度**: ①`readUint8Array()` のたびに `makeS3Client()` を呼び接続を使い回せていない ②zip.js の chunkSize が**既定 64KiB のまま**(`configuration.js` 実測)で compressed 15.73MB の PDF が**約 241 往復**。→ **S3Client は Reader の constructor で 1 個だけ**・**`chunkSize = S3_RANGE_CHUNK_BYTES = 16MiB`**。**timeout は 1 秒も伸ばしていない**。**ZIP 全体を 1 回で GET することにはならない** — 読み出し長は `min(chunkSize, そのエントリの compressedSize − 読んだ分)` で頭打ち・`getEntries()` は chunkSize を通らず EOCD 探索の 22+65535=**65,557 バイト**が上限 (`zip-reader.js:1589`)。実測 (21.25MB エントリ): エントリの Range GET **327→4 回** / 開くときは **2 回・最大 65,557B のまま** / **読んだ総バイト数は同じ**(余計に取っていない)。安全装置は 1 つも変更なし。検証 `verify:ad-hoc-archive` 85→**110**・退行注入 6 種 (**範囲検査を消しても別の TypeError が出るので「何かが throw した」では見抜けない** → 自分の guard の文言まで見る) |
